...
"""
API v3, that were deprecated in 9.0.
...
class GitlabClientV3:
    ...
    def get_project_tree(self, project_name: str):
        ...
        httpResponsResult = self.doRequest.get(
            '/projects/%s/repository/tree%s' % (project_id, self._path_private_token))
    ...
    def get_project_file(self, project_name, ref, file_path):
        ...
        httpResponsResult = self.doRequest.get(
            '/projects/%s/repository/files%s' % (project_id, self._path_private_token), params=params)
        return ujson.loads(httpResponsResult.body)

    def update_project_file(self, project_name: str, branch_name: str, file_path: str, content: str,
                            commit_message: str):
        """
        @param project_name:
        @param branch_name:
        ...
        """
        project_id = self._get_project_id(project_name)
        params = {'file_path': file_path, 'branch_name': branch_name, 'content': content,
