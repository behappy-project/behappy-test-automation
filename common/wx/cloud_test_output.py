...
class MiniTestApi:
    ...
    def __init__(self, user_token, group_en_id, send_key):
        self.token = user_token  # 需要填写自己的token
        self.group_en_id = group_en_id  # 项目英文ID
        self.send_key = send_key  # send_key
    ...
    def sendMessge(self, report_url, create_time, finish_time, total_case_num, success_case_num, send_key):
        """
        :return:
        """

        Webhook = f"https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key={send_key}"
...
if __name__ == '__main__':
    minitest_client = MiniTestApi('xx', 'xx', 'xx')
    # logger.debug("开始创建测试任务")
